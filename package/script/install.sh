...
function install() {
    ...
    local _input_path=$(convert_install_path "${input_install_path}")
    install_tool && register_uninstall ${install_path} && update_install_info \
        && install_subpackage ${install_path}/${MODULE_NAME} ${_input_path}
    if [ $? -eq 0 ]; then
