...
private:
    __aicore__ inline void InitVar(GM_ADDR tilingData)
    {
        ...
        workSpaceSize_ = tileNum_ * sizeof(float);
        workSpaceOffset_ = tileOffset_ * sizeof(float);
    }

    __aicore__ inline void InitOpBuffer()
    {
        uint32_t buffSize = sizeof(float) + numClass_ * sizeof(int64_t) + numClass_ * sizeof(float);
        pipe_.InitBuffer(this->calcBuf_, buffSize);
        size_t bufOffset = 0;
        this->reduceSumLocal_ = this->calcBuf_.template Get<float>(1);
        bufOffset += sizeof(float);
        this->labelOneHotLocal_ = this->calcBuf_.template GetWithOffset<int64_t>(numClass_, bufOffset);
        bufOffset += sizeof(numClass_ * sizeof(int64_t));
    ...
    __aicore__ inline void CopyIn(int32_t progress)
    {
        ...
        DataCopy(logitLocal, logitGm_[progress * numClass_], numClass_);
        DataCopy(labelLocal, labelGm_[progress], 1);
    ...
    {
        if (blockIdx_ == 0) {
            uint32_t copyNums = (workSpaceSize_ * blockDim_) / sizeof(float);
            LocalTensor<float> logitLocal = inQueLogit_.AllocTensor<float>();
