...
![image](../../pic/c0a326b0db9a4090093b24cb8172eb8c_559x304.png)
...
![image](../../pic/045770052e13e209fbd1c08d7f823b4f_796x564.png)
...
其中劫持ascendcl_impl、runtime目的是为了采集算子的性能数据；（当前版本兼容runtime和aclrt两种启动方式，当rt接口日落后则可以删除对runtime接口的劫持和调用）
...
![image](../../pic/65735dda83950e7537ac39b9b5814ca6_676x330.png)
...
![image](../../pic/a0d7778dc45c84bd7ea9ec128dc12c64_606x534.png)
...
![image](../../pic/8dd1dc24b25bf85bbcf95909f0596fe9_491x499.png)
...
![image](../../pic/8a89fbabc5733e1a7aac5a054dcb2020_951x890.png)
...
![image](../../pic/3abb8bd5fbcd5ab405175535888d270a_865x509.png)
...
![image](../../pic/d56db36d46d521390d256af71d135b82_714x738.png)

#### 4.1.3 数据可视化模块

##### 4.1.3.1 概述

![image](../../pic/50769b9f211c0b12606cdfc20451937e_755x247.png)
...
![image](../../pic/aa18e26188739b739a336b9f02b81b93_741x620.png)

**分核流水图**
分核流水图只需要在每个core下面生成对应流水图JSON文件即可，在指令处理这里修改了UserMark的处理逻辑，单独从Datacenter中取出UserMark的数据单独解析，最后直接在对应的core目录下生成分核JSON文件。

![image](../../pic/42ce072b0176e22468f44988463b9602_634x732.png)

**整核流水图**

![image](../../pic/0445179afe113aca20d9c881bb2e7971_599x563.png)
...
![image](../../pic/edeab66075d8586d74141cb17b2c783e_440x245.png)

msopt仓编译时会生成libprofplugin_memorychart.so,基础组件使用该库实现动态插桩相关功能，具体操作详见下文。

##### 4.1.4.3 逻辑视图

![image](../../pic/718f6c588384db46683f20bf40a856bb_496x374.png)
...
![image](../../pic/c1018e879215315155a838be67be24af_887x556.png)
...
![image](../../pic/f02a02a67847d793c2ca85ddeebd034b_971x438.png)
...
![image](../../pic/cf1aff15b08bb3fe4b7634e5202de28e_606x467.png)
...
| --export                   | 指定包含单算子仿真结果文件夹，直接对该仿真结果进行解析               | 取值范围为1\~100之间的整数                                                                                                         | 仿真       |
| --core-id                  | 可使用--core-id参数指定部分逻辑核的id                                | 取值范围为[0,49]                                                                                                                   | 上板、仿真 |
| --timeout                  | 设置--timeout参数缩短算子运行时长并获取必要流水信息                  | 取值范围为1\~100之间的整数                                                                                                         | 仿真       |
...
![image](../../pic/e798b43938a390ec8513f21c4c077819_639x840.png)
...
![image](../../pic/1a12bc5385fc2d2751bf3684dc8ef711_606x314.png)
...
![image](../../pic/428ea3fa9e3357494c0695c49c79c5e5_756x348.png)
...
3. 用户调用rtRegisterAllKernel接口。此时仿真采集需要记录.o信息。当前先将.o落盘，并使用map<const void *, std::string>记录.o信息，一个hdl对应一个.o落盘地址。
...
1. kernel级重放
   ...
   ![image](../../pic/33e097268e3e2fec3e00bf7d3f11943b_802x293.png)
2. 应用级重放
...
3. 范围级重放
   ...
   ![image](../../pic/cee27f22cc2250ad3a5581dd027d64ec_705x586.png)

##### 5.1.1.3.1 仿真采集

**劫持逻辑

![image](../../pic/1f3babf8f320f45808062c10f4829f0c_676x393.png)

仿真库间的依赖：

![image](../../pic/4f2aa7604d013cd8200e9d3dec0e0cb3_554x139.png)
...
1. 与Camodel交互
   ...
   ![image](../../pic/bde468e9c0c6b148a76c2a4dba5f4957_590x188.png)
2. 与工具侧交互
   ...
   ![image](../../pic/c05e7daabb8ce6bfefaf9d7f981b3510_616x909.png)

**动态插桩**
函数调用关系：

![image](../../pic/0a88cded14362a8b1296f411041af697_281x511.png)
...
![image](../../pic/c6dde60847f0fc6d376b5c21cbda745e_471x370.png)
...
![image](../../pic/ee49ee40ab6421501678387083daf5bd_1010x619.png)
...
![image](../../pic/97bc6c3118b9d9963b644bf344bc47a9_1190x591.png)
...
![image](../../pic/4d3d83b9ba3452f568ca875a6fba0f22_600x543.png)
...
4. 当算子运行结束后会发送Stop信息，收到后会调用Shutdown函数，关闭数据等待，此时插件会停止等待新数据并进行后续操作。
   ...
   ![image](../../pic/3cb0ff74e5f415ca79f81475417e9b8c_549x424.png)
   ...
   **dump文件解析**
   ...
   ![image](../../pic/c25a835f49bbef90ca85052b158dcc94_543x598.png)
...
3. ProConfig记录工具侧的信息，例如需要采集的PMU所有算子共用，所有全局只需要一个。
   ...
   ![image](../../pic/949ac9f03680c24e723b9c6db7daf0cd_710x346.png)
...
![image](../../pic/c8f24f9a8445ecaa4bf74839170d83a0_1105x705.png)
