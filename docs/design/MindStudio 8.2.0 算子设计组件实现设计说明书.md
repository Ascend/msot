# MindStudio 8.2.0 算子设计组件实现设计说明书

## 1. 概述

MSKPP（Kernel Performance Prediction），支持用户输入算子表达式，进而预测算子在这一算法实现下的性能上限。
...
### 2.3 CATLASS模板库一键式自动寻优

| 类型     | 功能清单                                                     | 功能描述                                                     | 支撑的系统功能                           |
| -------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ---------------------------------------- |
| 业务功能 | 支持指定条件过滤模板库搜索空间并生成部分用例                 | 在模板库代码仓中，支持根据输入过滤条件，生成过滤后的算子用例代码。 | 支持基于模板库提供的搜索空间生成用例代码 |
| 业务功能 | 支持通过命令行基于模板库搜索空间完成用例代码的生成           | 在模板库代码仓中，支持解析预设的搜索空间，生成对应算子的用例代码及编译脚本 | 支持基于模板库提供的搜索空间生成用例代码 |
| 业务功能 | 支持生成的用例代码自动化编译                                 | 在模板库代码仓中，支持生成的算子用例代码一键式编译           | 支持基于模板库提供的搜索空间生成用例代码 |
| 业务功能 | 模板库寻优工具支持命令行一键式执行搜索空间用例，获取用例性能数据 | 在模板库代码仓，新增自动寻优工具，支持从命令行输入相应信息，使工具运行算子测试用例，并获取性能数据 | 支持一键式执行模板库用例采集性能数据     |

## 3. 软件实现设计目标分析与关键要素设计

### 3.1 整体设计目标分析

1、代码易扩展：算子性能建模依赖已有的指令性能数据，昇腾基础指令上百种，如果方便用户扩展指令达到对使用该指令的算子进行建模，提供开放易用的指令注册功能是一个关键目标；提供标准的tiling函数与kernel函数的下发代码生成、编译、运行能力，面向不同算子工程易扩展。
...
![image](../pic/3aaa6dee2c7d6ea527eaa3dae5f60cef_3197x1388.png)
...
![image](../pic/6e93a35427501292e4f17d7f2de9382d_2466x1697.png)
...
代码生成子模块静态结构框图，
![image](../pic/c3c78f4d6300e662e23871fcd79afa78_3213x1360.png)

编译与运行模块静态结构框图，
![image](../pic/fb438072c2aec562011154c22a6caef1_3157x2450.png)

#### 4.1.3 CATLASS模板库一键式自动寻优

##### 4.1.3.1 概述

模板库一键式寻优组件，主要提供：模板库算子用例生成、编译、批量运行及性能采集等功能，输出全量用例的性能数据，为算子 Tiling 参数的寻优提供参考依据。

实现元素功能描述：

1. 支持基于模板库提供的搜索空间生成用例代码；
2. 支持一键式执行模板库用例采集性能数据；

根据上述功能实现分解出以下软件实现子模块：
...
- 算子开发者：通过一键式寻优工具进行模板库算子tiling参数快速寻优。

##### 4.1.3.2 上下文视图

CATLASS模板库一键式自动寻优工具 msTuner_CATLASS 在CATLASS仓提供源码与编译方式，向开发者提供命令行接口，依赖如下组件：

1. runtime，运行时库实现算子下发；
2. AscendCL，设备上下文管理；
3. bisheng编译器，算子用例代码编译；
4. AscendHAL，性能数据采集；
5. CATLASS模板库算子接口；

![image](../pic/33289aa297948bec5f30a6be74ba4e4e_2385x2450.png)

##### 4.1.3.3 逻辑视图

从功能上需划分以下软件实现模块：

1. 代码生成模块，解析搜索空间配置并生成全量算子用例；
2. 代码编译模块，编译全量算子用例代码生成动态库；
3. 命令行解析模块，获取算子动态入参；
4. 算子下发模块，初始化算子资源，调用算子下发接口；
5. 性能采集模块，采集算子运行性能数据；

代码生成、编译模块会因开发者修改外部搜索空间配置后，反复重新编译，因此其最终部署态作为一个独立交付件（libcatlass_kernels.so动态库）与其他模块（mstuner_catlass）区分，他们通过一系列内部接口进行协作。

![image](../pic/b3f8f19ddfb6ed5c738c3a6946d942f7_5197x1919.png)

软件单元清单：

| 软件单元   | 描述                               | 外部接口       | 内部接口               | 关系描述                                                     |
| ---------- | ---------------------------------- | -------------- | ---------------------- | ------------------------------------------------------------ |
| 代码生成   | 解析搜索空间配置并生成全量算子用例 | Manifest接口类 | 算子获取接口、下发接口 | 解析外部输入的搜索空间配置，生成算子用例代码，提供给代码编译模块使用 |
| 代码编译   | 编译全量算子用例代码生成动态库     | cmake接口      | /                      | 使用生成的算子用例代码，并行完成编译生成动态库，向算子下发模块提供算子获取接口与下发接口。 |
| 命令行解析 | 获取算子动态入参                   | 命令行接口     | /                      | 解析外部输入的命令行入参，获取算子所需的动态参数             |
| 算子下发   | 初始化算子资源，调用算子下发接口   | /              | /                      | 根据算子动态入参，初始化算子所需资源，调用算子获取与下发接口，完成算子下发 |
| 性能采集   | 采集算子运行性能数据               | 性能数据文件   | /                      | 在算子下发的同时，调用驱动接口开启profiling数据通道，获取所有算子运行性能数据 |

##### 4.1.2.4 软件实现单元设计

代码生成、代码编译模块静态结构框图：
![image](../pic/87f2d98651ac937fce3278317e9ed137_4260x2107.png)

![image](../pic/d63d9f67f66437fe1b58020c002bc4df_4572x2354.png)

命令行、算子下发、性能采集模块静态结构框图：

![image](../pic/47ce273ef36441752f86d5205b4285ab_3389x2086.jpg@900-0-90-f.jpg)
...
mskpp提供简便易用的DSL语言帮助用户快速构建自己的算子实现想法，DSL以一系列接口进行呈现，主要分为基础功能接口和特定指令接口。
...
1、kpp对外提供接口应保持职责单一，不同接口承担单独的算子行为；
...
指令调度单元：指定调度单元的核心功能是接收外部指令数据，按照芯片行为进行建模调度，生成指令的调度起止时间；
...
![image-20251212142332000](../pic/ee09dc2b587c3deef1d8575fed8e9864_1703x1267.png)
...
![image](../pic/0e6fab0c918e34303ce4e77d445c8344_1239x1003.png)
...
方案二：
![image](../pic/171173b8b170d56a4559b663545b5603_1553x1097.png)
...
#### 4.2.3 CATLASS模板库一键式自动寻优

##### 4.2.3.1 总体目标

msTuner_CATLASS工具为CATLASS模板库算子提供一键式tiling参数寻优，开发者仅需通过简单命令行命令即可完成寻优操作，同时向CATLASS模板库开放搜索空间配置接口。

##### 4.2.3.2 设计目标

命令行接口作为外部接口直接面向开发者，应清晰定义接口提供的功能范围，同时为应对未来可能扩展的场景预留灵活可扩展的参数。内部子模块间的接口应尽量简洁、减少依赖。

##### 4.2.3.3 设计约束

因CATLASS模板库算子在编译期会检查tiling参数的合法性，所以需要约束外部输入的tiling参数搜索空间配置满足其合法性校验。

##### 4.2.3.4 技术选型

调研友商CUTLASS的寻优接口方案如下：

1. 基于搜索空间完成用例代码生成：

```
$ cmake .. -DCUTLASS_NVCC_ARCHS="60" -DCUTLASS_LIBRARY_KERNELS=all
```
...
基于CATLASS一键式寻优接口界面设计如下图所示，

![image](../pic/b8400495b3016e8911171dfc56719184_1271x1145.png)
...
代码生成模块，解析来自外部的搜索空间配置，在约定目录下生成算子用例代码，提供给代码编译模块。

其向外部开发者提供CMake接口，其中`CATLASS_LIBRARY_KERNELS`变量可用于过滤生成的算子类型：

```
cmake .. -DCATLASS_LIBRARY_KERNELS=basic_matmul
```

代码生成目录固定在 `build/tools/library/generated`，供代码编译模块直接使用，目录结构设计如下：

![image-20251212141721910](../pic/1b7b705300e0a3da4c161abefcb2cb9e_1260x1047.png)
...
代码编译模块使用代码生成模块生成的算子代码文件，完成算子用例库的编译。
...
算子用例数量较多（通常大于1000），编译接口需支持使用bisheng编译器进行多核并行编译，为保证算子用例性能与CATLASS模板库原有example保持一致，编译选项配置需与example完全一致。

##### 4.2.3.7 软件单元--命令行解析模块

命令行解析模块接收开发者的命令行输入，解析为算子动态入参。

命令行参数设计如下，参数均为非必选参数，其中一些在大部分场景均会被使用的参数被设置了默认值：
...
命令行解析结果可通过内部接口`CommandLineParser::Get`提供给算子下发模块使用。
...
算子下发模块从命令行解析模块获取开发者输入的算子动态参数，完成算子的资源初始化，并调用算子获取、下发接口，完成算子用例的批量下发。
...
class OpConfig {
public:
    explicit OpConfig(const Library::OperationDescription &desc) : kind_(static_cast<uint32_t>(desc.kind)) {}
    static std::shared_ptr<OpConfig> GetOpConfig(const Library::OperationDescription &desc);
...
##### 4.2.3.9 软件单元--算子性能采集模块

算子性能采集模块在算子下发的同时，调用CANN驱动接口，获取下发算子的运行性能数据。
...
public:
    ...
    inline void RegisterCallback(CallBackFunc callBackFunc);
    bool Start();
    void Stop();
...
#### 4.5.3 CATLASS模板库一键式自动寻优

##### 4.5.3.1 安全设计目标

一键式寻优提供的命令行接口应保证开发者使用时，拦截不合法入参，运行后程序状态均被正常处理，安全退出。
...
| 模块名称   | 模块功能简要说明                           | 设计域高风险模块分析                                                           | 对应代码目录                   | 语言类型 | 备注 |
| ------------ | -------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------- | ---------- | ------ |
| 代码生成   | 解析外部搜索空间配置，生成算子用例代码     | 代码生成模块会本地生成大量算子代码文件，需防范文件生成路径被劫持注入。         | catlass/tools/library/scripts/ | Python   |      |
| 命令行解析 | 解析开发者通过命令行参数输入的算子动态入参 | 算子动态入参值与算子资源申请强相关，需校验动态入参的范围是否满足当前硬件限制。 | catlass/tools/tuner/           | C++      |      |
| 性能采集   | 采集下发算子的性能数据并落盘               | 性能数据落盘路径由外部输入，需校验输入路径是否安全。                           | catlass/tools/tuner/           | C++      |      |

###### 4.5.2.3.2 高风险API识别

| 高风险API              | 接口说明                 | 高风险接口函数分析                                                     | 对应代码目录                                      | 语言类型 | 备注 |
| ------------------------ | -------------------------- | ------------------------------------------------------------------------ | --------------------------------------------------- | ---------- | ------ |
| CommandLineParser::Get | 从命令行解析外部输入参数 | 外部通过命令行输入的字符串需转换为其他数据类型，需重点防护转换异常场景 | catlass/tools/tuner/src/command_line_parser.cpp | C++      |      |
| Metrics::Dump          | 性能数据落盘             | 创建文件时会清理原始同名文件，需重点防护原文件清理的合法操作           | catlass/tools/tuner/src/metrics.cpp               | C++      |      |

##### 4.5.2.4 代码实现安全防范处理

**1. 输入验证**

对命令行的外部输入进行严格验证，确认输入数据的合法性，防止恶意攻击。

需重点防范：

1. 输入数据类型与期望格式是否一致，避免出现类型转换时数字翻转、转换异常等问题；
2. 输入数据范围是否合法，避免出现过大入参导致算子使用其进行资源初始化时，耗尽硬件资源等问题；

**2. 错误和异常处理**合理的错误和异常处理机制能够保证命令行工具能在异常情况下以可控的方式终止，并向用户返回恰当的错误信息。

当批量运行算子时，因意外因素某几个算子执行失败，应当隔离该异常，保持其他算子正常运行，并获取正确的性能数据，并抛出相应的异常提示用户。
...
使用真实的数据来实现组件或者端到端的验收测试。构造与外部实现相同的mskpp-DSL语言来进行指令的任务创建与调度并校验生成数据结果。由于以上过程不存在
随机性，因此结果数据可校验。
...
#### 4.6.3 CATLASS模板库一键式自动寻优

##### 4.6.3.1 设计目标

本章节定义CATLASS模板库一键式自动寻优工具的开发者测试关键要素，包括软件可测试性设计和测试分层策略。
...
| 分层 | 测试类型 | 测试对象           | 测试价值                         |
| ------ | ---------- | -------------------- | ---------------------------------- |
| ST   | 系统测试 | 一键式寻优整体功能 | 端到端验证一键式寻优整体功能     |
| UT   | 单元测试 | 各模块内部函数、类 | 验证最小实现单元工作是否符合预期 |

##### 4.6.3.5 关键测试技术方案

1. 测试工程设计

Python代码使用pytest进行，CPP代码使用googletest和mockcpp进行。
...
1. 运行环境
   Python：pytest
   CPP：googletest、mockcpp
2. 测试替身设计不涉及。
3. DSL设计不涉及。
4. 数据构造设计使用README中展示的样例作为端到端测试用例，输出结果可预测。
5. 夹具设计不涉及。
6. 匹配器设计不涉及。
...
![image](../pic/d167a1df00f01576fb2cadc30fb35bd6_3097x3847.png)

#### 5.1.3 CATLASS模板库一键式自动寻优

##### 5.1.3.1 设计目标

交互应遵循一键式设计，前台仅需开发者完成一次输入，后端完成全部流程并输出结果。
...
![image](../pic/7167fabb966943a6bc2aad89b62634ea_2879x2454.png)
算子下发、性能采集模块交互流程：
![image](../pic/138b5690c72d4243ae747333414f9b0b.png)
...
![image](../pic/8fc9562a698616b7dd89cd087f5dc589_2571x1247.png)
